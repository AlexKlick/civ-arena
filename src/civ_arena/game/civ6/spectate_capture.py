...
class SpectateTransport:
    ...
    async def read_raw(self, lua: str) -> list[str]:
        ...
        # CAP-R1 #5: recorder commands fullmatch the single-call regex
        # (no prefix check anywhere). M4 §4b: the two SPECW world reads
        # join as EXACT full-command equality. Both paths exact — a
        # one-character drift in either Lua re-tightens by construction.
        if (self._RECORDER_CALL.fullmatch(lua) is None
                and lua not in self._allowed_world_reads()):
            self.census["rejected"] += 1
            raise RecorderCapabilityError(
                f"spectator read_raw is restricted to the exact ambient-"
                f"window recorder calls and the exact SPECW world reads, "
                f"refusing: {lua[:80]!r}")

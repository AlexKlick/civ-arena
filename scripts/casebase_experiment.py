...
def _validity_problems(rows: list[dict], requested_turns: int) -> list[str]:
    """Inference validity gate (Codex M20b B1, backported from the bandit
    runner this one seeded): inference is refused unless ZERO dirty rows
    ...
    display-only numbers. The historical exp-M19b legs were all-clean
    (0 dirty, all rows at their requested horizons, complete pairs), so
    their recorded verdicts stand under this gate."""
    problems: list[str] = []
...
def paired_report(rows: list[dict], requested_turns: int) -> str:
    ...
    if problems:
        ...
        out.append(f"case wins {wins} / base wins {len(clean) - wins} "
                   f"/ ties {ties}")
        if clean:
        ...
        for side in sorted({k[1] for k, _, _ in pairs}):
            ...
            out.append(f"side {side}: case {w} / base {lost} "
                       f"/ tie {len(sd) - w - lost}")

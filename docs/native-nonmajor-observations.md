...
The adapter's visibility remains its existing own-unit/city radius approximation, not native line-of-sight or fog proof. It may disagree with the UI around terrain/occlusion. ~~`cities_read()` still enumerates major civilizations, so this unit-only change does not add city-state city observations.~~ STRUCK 2026-09-08 (M4): `cities_read(extended=True)` now enumerates `PlayerManager.GetAlive()` — city-state cities reach the omniscient observation (CITIES\|2, `CITIES|1` stays majors-only), and the spectator world's `roster_read` carries minor identity; see `docs/spectator-capture.md` §"M4". No diplomatic war-state classification or nonmajor-turn control is included. No full release gate was run in this isolated lane; integration owns that gate.

...
- Caps: 32 goals / 64 predictions / 64 lessons per player.
- Scoring contract (M11): single-direction — a metric claim is `met` when
  the observed value ≥ target; "at most" phrasing is text, scored
  `self_assess`. `subject_id` claims and metric-less claims are
  `self_assess`: the arena cannot falsify what vision cannot see. Overdue
  goals and unreviewed predictions stay due until amended or superseded.
...
`FOREIGN_CITY_FIELDS`, never more than the player saw. `replay._strip`
...
nothing new. Cost on the 40-turn reference run: ~+40–70 KB.
...
- live store == `from_log(log)` / prefix-wise / after resume / after
  model-free replay;
- claim writes never touch game state; oversized claims die at the LLM
  runtime with zero events (replay preservation);
- belief fields ⊆ projection allowlists; hidden entities stay absent;

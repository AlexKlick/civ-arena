...
def test_client_parse_armor():
    ...
    for bad in ["not a dict", {"content": [5]},
                {"content": [{"type": 7}]},
                {"content": [], "usage": {"input_tokens": "x"}}]:
        with pytest.raises(ModelUnavailable, match="malformed"):
            client._parse(bad)
    # Provider-compatible whole-string content is intentionally accepted and
    # normalized to one typed text block (the live GLM compatibility shape).
    reply = client._parse({"content": "no"})
    assert reply.content == [{"type": "text", "text": "no"}]
